...
class PushUpApp {
    ...
    initElements() {
        ...
        this.modalShareRoutines = document.getElementById('modal-share-routines');
        ...
        if (this.modalShareRoutines) {
            this.modalShareRoutines.classList.add('hidden');
            this.modalShareRoutines.style.display = 'none';
        }
    }
    ...
    initEventListeners() {
        ...
        // Share & Import Routines Modal
        const btnShareRoutines = document.getElementById('btn-share-routines');
        if (btnShareRoutines) {
            btnShareRoutines.addEventListener('click', () => this.openShareModal());
        }
        const btnCloseShareModal = document.getElementById('btn-close-share-modal');
        if (btnCloseShareModal) {
            btnCloseShareModal.addEventListener('click', () => this.closeShareModal());
        }
        const btnCancelImport = document.getElementById('btn-cancel-import');
        if (btnCancelImport) {
            btnCancelImport.addEventListener('click', () => this.closeShareModal());
        }
        ...
        if (tabBtnExport && tabBtnImport) {
            ...
            tabBtnImport.addEventListener('click', () => {
                ...
                if (tabContentImport) {
                    ...
                    tabContentImport.classList.add('active');
                }
                if (tabContentExport) {
                    tabContentExport.classList.add('hidden');
                    tabContentExport.classList.remove('active');
                }
            });
        }
        ...
        if (btnCopyShareLink) {
            btnCopyShareLink.addEventListener('click', () => this.copyShareLink());
        }
        const btnCopyShareCode = document.getElementById('btn-copy-share-code');
        if (btnCopyShareCode) {
            btnCopyShareCode.addEventListener('click', () => this.copyShareCode());
        }
        const btnDownloadRoutineJson = document.getElementById('btn-download-routine-json');
        if (btnDownloadRoutineJson) {
            btnDownloadRoutineJson.addEventListener('click', () => this.downloadRoutineJson());
        }
        ...
        if (btnTriggerFileImport && fileImportJson) {
            btnTriggerFileImport.addEventListener('click', () => fileImportJson.click());
            fileImportJson.addEventListener('change', (e) => this.handleFileImport(e));
        }
        const btnConfirmImport = document.getElementById('btn-confirm-import');
        if (btnConfirmImport) {
            btnConfirmImport.addEventListener('click', () => this.confirmImportRoutines());
        }
        ...
        if (this.modalShareRoutines) {
            this.modalShareRoutines.addEventListener('click', (e) => {
                if (e.target === this.modalShareRoutines) this.closeShareModal();
            });
        }
    ...
    // ================= SHARE & IMPORT ROUTINES =================
    openShareModal() {
        if (!this.modalShareRoutines) return;
        ...
        const copyToast = document.getElementById('share-copy-toast');
        if (copyToast) copyToast.classList.add('hidden');

        const fileNameStatus = document.getElementById('import-file-name');
        if (fileNameStatus) fileNameStatus.textContent = '';

        this.modalShareRoutines.classList.remove('hidden');
        this.modalShareRoutines.style.display = 'flex';
    }
    ...
    showShareToast(msg) {
        ...
        if (!toast) return;
        toast.textContent = msg || '✓ Đã sao chép thành công!';
    ...
    decodeRoutines(raw) {
        ...
        if (!str) throw new Error('Vui lòng dán mã hoặc đường dẫn giáo án!');

        // If user pasted a full URL with hash or query
        if (str.includes('#import=')) {
            str = str.split('#import=')[1];
        } else if (str.includes('?import=')) {
            str = str.split('?import=')[1].split('&')[0];
        }
        ...
        // Try decoding Base64 UTF-8
        try {
            const decodedUri = decodeURIComponent(Array.prototype.map.call(atob(str), (c) => {
    ...
    copyShareCode() {
        try {
            const encoded = this.encodeRoutines(this.routines);
            navigator.clipboard.writeText(encoded).then(() => {
                this.showShareToast('✓ Đã sao chép mã giáo án vào clipboard!');
            }).catch(() => {
                prompt('Mã giáo án của bạn (hãy sao chép):', encoded);
            });
        } catch (e) {
            alert('Không thể tạo mã chia sẻ: ' + e.message);
        }
    }
    ...
    copyShareLink() {
        try {
            ...
            navigator.clipboard.writeText(url).then(() => {
                this.showShareToast('✓ Đã sao chép liên kết chia sẻ!');
            }).catch(() => {
                prompt('Đường dẫn chia sẻ (hãy sao chép):', url);
            });
        } catch (e) {
            alert('Không thể tạo liên kết chia sẻ: ' + e.message);
        }
    }

    downloadRoutineJson() {
        try {
            const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(this.routines, null, 2));
            const downloadAnchor = document.createElement('a');
            downloadAnchor.setAttribute("href", dataStr);
            downloadAnchor.setAttribute("download", "giao-an-pushup.json");
            document.body.appendChild(downloadAnchor);
            downloadAnchor.click();
            downloadAnchor.remove();
            this.showShareToast('✓ Đã bắt đầu tải file giao-an-pushup.json!');
        } catch (e) {
    ...
    handleFileImport(e) {
        ...
        const nameStatus = document.getElementById('import-file-name');
        if (nameStatus) nameStatus.textContent = 'Đã chọn: ' + file.name;

        const reader = new FileReader();
        reader.onload = (event) => {
            const textarea = document.getElementById('textarea-import-code');
            if (textarea) {
    ...
    confirmImportRoutines() {
        const textarea = document.getElementById('textarea-import-code');
        const rawContent = textarea ? textarea.value.trim() : '';

        if (!rawContent) {
            alert('Vui lòng dán mã chia sẻ hoặc chọn file .json!');
            return;
        }

        try {
            const importedList = this.decodeRoutines(rawContent);
            if (!Array.isArray(importedList) || importedList.length === 0) {
                throw new Error('Dữ liệu giáo án không hợp lệ hoặc rỗng!');
            }

            // Validate structure
            for (let i = 0; i < importedList.length; i++) {
                const item = importedList[i];
                if (!item.name || !Array.isArray(item.sets) || item.sets.length === 0) {
                    throw new Error(`Dữ liệu mục thứ ${i + 1} không đúng cấu trúc giáo án.`);
                }
            }
            ...
            if (mode === 'replace') {
                this.routines = importedList;
            } else {
                // Append mode: generate new unique ids
                importedList.forEach(item => {
                    const newItem = JSON.parse(JSON.stringify(item));
                    newItem.id = 'imported-' + Date.now() + '-' + Math.random().toString(36).substr(2, 4);
            ...
            this.renderRoutines();
            this.closeShareModal();
            alert(`✓ Đã nạp thành công ${importedList.length} ngày giáo án vào máy!`);
            if (textarea) textarea.value = '';
        } catch (e) {
            alert('Không thể nhập giáo án: ' + e.message);
        }
    }
    ...
    checkUrlImport() {
        ...
        if (hash && hash.includes('import=')) {
            importPayload = hash.split('import=')[1];
        } else if (search && search.includes('import=')) {
            const urlParams = new URLSearchParams(search);
            importPayload = urlParams.get('import');
        }

        if (!importPayload) return;

        try {
            const routines = this.decodeRoutines(importPayload);
            if (Array.isArray(routines) && routines.length > 0) {
                setTimeout(() => {
                    if (confirm(`Phát hiện đường dẫn chia sẻ có ${routines.length} ngày giáo án! Bạn có muốn nạp vào máy ngay bây giờ không?`)) {
                        this.routines = routines;
                        this.saveRoutines();
                        this.renderRoutines();
                        alert(`✓ Đã cập nhật ${routines.length} ngày giáo án vào ứng dụng!`);
                    }
            ...
            console.warn('Lỗi đọc giáo án từ URL:', e);
        }
    }
